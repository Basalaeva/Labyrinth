...
""")
        ...
        while !game.gameOver {
            ...
            switch true { //производится первичная обработка команды: проверка синтаксиса(наличие указанных дверей и итемов в комнате проверяется в модели), если команда определилась, выполняется
